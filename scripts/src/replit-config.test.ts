...
test("status-page keeps the only committed preview artifact while deploy still centers on root .replit", async () => {
  const apiServerArtifactToml = path.join(
    repoRoot,
    "artifacts",
    "api-server",
    ".replit-artifact",
  ...
  const statusPageArtifactToml = path.join(
    repoRoot,
    "artifacts",
    "status-page",
    ".replit-artifact",
  ...
  await assert.rejects(
    access(apiServerArtifactToml),
    /ENOENT/,
    `${apiServerArtifactToml} should not exist in the repo`,
  );
...
test(".gitignore keeps api-server artifact metadata ignored while allowing the committed status-page preview artifact", async () => {
  ...
  assert.match(gitignore, /^!artifacts\/status-page\/\.replit-artifact\/$/m);
  assert.match(gitignore, /^!artifacts\/status-page\/\.replit-artifact\/artifact\.toml$/m);
});
...
test("replit.md keeps deploy scoped to the root entrypoint while allowing the status-page preview artifact", async () => {
...
test("upload prompt tells AI to work from the current Git-imported project root", async () => {
  ...
  assert.match(prompt, /artifacts\/status-page\/\.replit-artifact\/artifact\.toml/);
